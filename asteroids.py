...
class Sprite:
    ...
    def collide(self, other_object):
        x1 = self.pos[0]
        y1 = self.pos[1]
        x2 = other_object.get_position()[0]
        y2 = other_object.get_position()[1]
...
def draw(canvas):
    ...
    score_update = group_group_collide(missile_group, rock_group) * 10
    score += score_update
...
def rock_spawner():
    ...
    spawn_distance1 = rock_pos[0] - my_ship.get_position()[0]
    spawn_distance2 = rock_pos[1] - my_ship.get_position()[1]
    spawn_distance = math.sqrt(spawn_distance1 ** 2 + spawn_distance2 ** 2)
    if len(rock_group) <= 12 and spawn_distance > 2 * my_ship.get_radius():
...
def group_group_collide(group1, group2):
    count = 0
    for each1 in group1.copy():
        if group_collide(group2, each1):
            count += 1
            group1.discard(each1)
    return count
